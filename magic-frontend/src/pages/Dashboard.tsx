...
import { useDeleteQuiz, useQuizes, useGenQuiz } from "../shared/queries";
...
export const Dashboard: React.FC<Props> = (props: any) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);

  const { publicMetadata } = useUser(); //********** user roles *********
  ...
  return (
    <div className="pb-10">
      ...
      <div className="flex justify-between mb-4 flex-wrap">
        ...
        {/************  end roles condition add here  *****************/}


      </div>
      {data?.quizes.length > 0 && (
        <div className="bg-gray-50 rounded px-8 py-6 transition-all flex flex-col lg:flex-row items-center justify-between mb-4">
          <h2
            style={{ maxWidth: 500 }}
